    ...
    : IRequestHandler<GetStockOnHandQuery, ApiResponse<PagedResult<StockOnHandDto>>>
{
        ...
        GetStockOnHandQuery request, CancellationToken cancellationToken)
    {
        ...
        if (request.BelowMinimumOnly)
            query = query.Where(s => s.Quantity < s.RawMaterial.MinimumStockLevel);

        var search = request.Parameters.Search?.Trim();
        if (!string.IsNullOrEmpty(search))
        {
            query = query.Where(s =>
                s.RawMaterial.Code.Contains(search) ||
                s.RawMaterial.Name.Contains(search));
        }

        query = (request.Parameters.SortBy?.ToLowerInvariant(), request.Parameters.SortDirection?.ToLowerInvariant()) switch
        {
            ("material", "desc")  => query.OrderByDescending(s => s.RawMaterial.Name),
            ("material", _)       => query.OrderBy(s => s.RawMaterial.Name),
            ...
            _                     => query.OrderBy(s => s.RawMaterial.Name)
        };
        ...
        var items = await query
            ...
            .Select(s => new StockOnHandDto(
                ...
                s.RawMaterial.Code,
                s.RawMaterial.Name,
                s.RawMaterial.UnitOfMeasure.Code,
                s.WarehouseId,
                s.Warehouse.Code,
                s.Warehouse.Name,
                s.Quantity,
                s.RawMaterial.MinimumStockLevel,
