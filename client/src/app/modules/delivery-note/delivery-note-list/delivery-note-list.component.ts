...
export class DeliveryNoteListComponent implements OnInit {
  ...
  private buildForm(): void {
    this.form = this.fb.group({
      ...
      transportCompany: ['', Validators.maxLength(150)],
  ...
  save(): void {
    ...
    const baseFields = {
      ...
      plannedDeliveryDate: (v.plannedDeliveryDate as string) || null,
      vehicleNumber: (v.vehicleNumber as string)?.trim() || null,
      transportCompany: (v.transportCompany as string)?.trim() || null,
      driverName: (v.driverName as string)?.trim() || null,
